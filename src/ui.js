...
const ENCODER_STORY_PLACEHOLDERS = {
  general: "【ここにユーザーの物語を書く】",
  seeker: "【ここにユーザーの霊的体験・信仰・思想を書く】",
};
...
function axisGuideCard(result) {
  const section = card("時間の居場所と臨在", [
    "時間の人口は、物語の内容が過去・現在・未来のどこにあるかを示します。臨在スペクトルは、時間相同士がモデル上でどれだけ切り離されずに保持されているかを示します。",
    "これは「本当に臨在しているか」を直接測るものではありません。時間相どうしが、モデル上でどれだけ切り離されずに保持されているかを示す指標です。",
  ]);
  const ent = result.entanglement12;
  if (!ent) return section;
  section.append(simpleTable(["時間の居場所", "人口"], [
    ["過去", formatNumber(ent.time_populations?.past)], ["現在", formatNumber(ent.time_populations?.now)], ["未来", formatNumber(ent.time_populations?.future)],
  ]));
  section.append(simpleTable(["臨在スペクトル", "値"], [
    ["記憶の臨在", formatNumber(ent.presence_spectrum?.past_now)], ["予期の臨在", formatNumber(ent.presence_spectrum?.now_future)], ["跳越の永遠度", formatNumber(ent.presence_spectrum?.past_future)],
  ]));
  return section;
}
...
function distributionCard(result) {
  const section = card("B. 理想確率 / statevector probabilities", [
    "この章は、回路を最後まで通したあと、12の象徴のどこに重心が残ったかを示します。数値が大きい成分ほど、最終状態で存在感が強く残っています。",
...
function renderResults(measurement) {
  ...
  if (result.entanglement12) {
    const structure = result.entanglement12;
    const ent = card("三軸の結び", ["結びの強さはGHZ/W型分類ではなく、三軸の正規化エントロピー平均による粗い代替指標です。"]);
    ent.append(simpleTable(["指標", "値"], [
      ["結びの強さ", `${formatNumber(structure.binding_normalized)} (${structure.binding_label})`],
      ["主体軸エントロピー", formatNumber(structure.axis_entropies_bits?.subject)],
      ["顕現軸エントロピー", formatNumber(structure.axis_entropies_bits?.manifestation)],
      ["時間軸エントロピー", formatNumber(structure.axis_entropies_bits?.time)],
    ]));
    output.append(ent);
  }
  ...
  if (result.projected_2bit) {
    const projected = card("時間を畳んだ視点", [
      "過去・現在・未来の区別をいったん外し、a0+a1+a2のように同じ成分族を合算した4成分の俯瞰です。",
      "12Tで計算した結果を時間軸だけ畳み、3Qと共有する2×2の芯を見る補助表示です。",
    ]);
